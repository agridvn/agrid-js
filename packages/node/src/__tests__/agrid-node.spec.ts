import { Agrid, AgridOptions } from '@/entrypoints/index.node'
...
describe('Agrid Node.js', () => {
  ...
  describe('core methods', () => {
    it('should capture an event to shared queue', async () => {
      ...
      agrid.capture({ distinctId: '123', event: 'test-event', properties: { foo: 'bar' }, groups: { org: 123 } })
    ...
    it('should capture identify events on shared queue', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      agrid.identify({ distinctId: '123', properties: { foo: 'bar' } })
      jest.runOnlyPendingTimers()
      await waitForPromises()
    ...
    it('should handle identify using $set and $set_once', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      agrid.identify({ distinctId: '123', properties: { $set: { foo: 'bar' }, $set_once: { vip: true } } })
      jest.runOnlyPendingTimers()
      await waitForPromises()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
    ...
    it('should handle identify using $set_once', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      agrid.identify({ distinctId: '123', properties: { foo: 'bar', $set_once: { vip: true } } })
      jest.runOnlyPendingTimers()
      await waitForPromises()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
    ...
    it('should capture alias events on shared queue', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      agrid.alias({ distinctId: '123', alias: '1234' })
      jest.runOnlyPendingTimers()
      await waitForPromises()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
    ...
    it('should allow overriding timestamp', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      agrid.capture({ event: 'custom-time', distinctId: '123', timestamp: new Date('2021-02-03') })
      await waitForFlushTimer()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
        {
    ...
    it('should allow overriding uuid', async () => {
      ...
      agrid.capture({ event: 'custom-time', distinctId: '123', uuid })
      await waitForFlushTimer()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
        {
    ...
    it('should use default is set, and override on specific disableGeoip calls', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      const client = new Agrid('TEST_API_KEY', {
        host: 'http://example.com',
        disableGeoip: false,
        disableCompression: true,
      })
  ...
  describe('before_send', () => {
    ...
    it('should log when event is dropped in debug mode', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith('[Agrid]', "Event 'test-event' was rejected in beforeSend function")
      logSpy.mockRestore()
    })
  })
  ...
  describe('shutdown', () => {
    ...
    it('should shutdown cleanly', async () => {
      ...
      const shutdownPromise = (ph as any).shutdown()
  ...
  describe('groupIdentify', () => {
    it('should identify group with unique id', async () => {
      agrid.groupIdentify({ groupType: 'agrid', groupKey: 'team-1', properties: { analytics: true } })
      jest.runOnlyPendingTimers()
      await agrid.flush()
      const batchEvents = getLastBatchEvents()
      expect(batchEvents).toMatchObject([
        {
          distinct_id: '$agrid_team-1',
          event: '$groupidentify',
          properties: {
            $group_type: 'agrid',
            $group_key: 'team-1',
  ...
  describe('feature flags', () => {
    beforeEach(() => {
      ...
      const multivariateFlag = {
        ...
        filters: {
          groups: [
            {
              properties: [{ key: 'email', type: 'person', value: 'test@agrid.com', operator: 'exact' }],
              rollout_percentage: 100,
            },
            {
    ...
    it('should do getFeatureFlag', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      await expect(agrid.getFeatureFlag('feature-variant', '123', { groups: { org: '123' } })).resolves.toEqual(
        'variant'
      )
    ...
    it('should do isFeatureEnabled', async () => {
      expect(mockedFetch).toHaveBeenCalledTimes(0)
      await expect(agrid.isFeatureEnabled('feature-1', '123', { groups: { org: '123' } })).resolves.toEqual(true)
      await expect(agrid.isFeatureEnabled('feature-4', '123', { groups: { org: '123' } })).resolves.toEqual(undefined)
    ...
    it('should use minimum featureFlagsPollingInterval of 100ms if set less to less than 100', async () => {
      ...
      expect(agrid.options.featureFlagsPollingInterval).toEqual(100)
    })
    ...
    it('should use default featureFlagsPollingInterval of 30000ms if none provided', async () => {
      ...
      expect(agrid.options.featureFlagsPollingInterval).toEqual(30000)
    })
    ...
    it('manages memory well when sending feature flags', async () => {
      ...
      expect(Object.keys(agrid.distinctIdHasSentFlagCalls).length).toEqual(0)

      for (let i = 0; i < 100; i++) {
        const distinctId = `some-distinct-id${i}`
        await agrid.getFeatureFlag('beta-feature', distinctId)
        ...
        expect(Object.keys(agrid.distinctIdHasSentFlagCalls).length <= 10).toEqual(true)
      }
    })
    ...
    it('$feature_flag_called is called appropriately when querying flags', async () => {
      ...
      expect(
        await agrid.getFeatureFlag('beta-feature', 'some-distinct-id2', {
          groups: { x: 'y' },
          personProperties: { region: 'USA', name: 'Aloha' },
          disableGeoip: false,
        })
      ...
      expect(
        await agrid.getFeatureFlag('beta-feature', 'some-distinct-id23', {
          personProperties: { region: 'USA', name: 'Aloha' },
      ...
      expect(
        await agrid.getFeatureFlag('flags-flag', 'some-distinct-id2345', {
      ...
      expect(
        await agrid.isFeatureEnabled('flags-flag', 'some-distinct-id2345', {
          groups: { organization: 'org1' },
          personProperties: { region: 'USA', name: 'Aloha' },
        })
      ).toEqual(true)
  ...
  describe('evaluation environments', () => {
    ...
    it('should send evaluation environments when configured', async () => {
      ...
      const agridWithEnvs = new Agrid('TEST_API_KEY', {
        host: 'http://example.com',
        evaluationEnvironments: ['production', 'backend'],
        ...agridImmediateResolveOptions,
      })

      await agridWithEnvs.getAllFlags('some-distinct-id')
      ...
      await (agridWithEnvs as any).shutdown()
    })
    ...
    it('should not send evaluation environments when not configured', async () => {
      ...
      const agridWithoutEnvs = new Agrid('TEST_API_KEY', {
        host: 'http://example.com',
        ...agridImmediateResolveOptions,
      })

      await agridWithoutEnvs.getAllFlags('some-distinct-id')
      ...
      await (agridWithoutEnvs as any).shutdown()
    })
    ...
    it('should not send evaluation environments when configured as empty array', async () => {
      ...
      const agridWithEmptyEnvs = new Agrid('TEST_API_KEY', {
        host: 'http://example.com',
        evaluationEnvironments: [],
        ...agridImmediateResolveOptions,
      })

      await agridWithEmptyEnvs.getAllFlags('some-distinct-id')
      ...
      await (agridWithEmptyEnvs as any).shutdown()
    })
  })
  ...
  describe('getRemoteConfigPayload', () => {
    ...
    beforeEach(() => {
      ...
      // Initialize agrid with personalApiKey
      agrid = new Agrid('TEST_API_KEY', {
        host: 'http://example.com',
        fetchRetryCount: 0,
        disableCompression: true,
      ...
      requestRemoteConfigPayloadSpy = jest.spyOn(agrid as any, '_requestRemoteConfigPayload')
    })

    it('should throw error when personalApiKey is not provided', async () => {
      const agridWithoutKey = new Agrid('TEST_API_KEY', {
        host: 'http://example.com',
        fetchRetryCount: 0,
        disableCompression: true,
      })

      await expect(agridWithoutKey.getRemoteConfigPayload('test-flag')).rejects.toThrow(
    ...
    it('should return empty object when no payload is available', async () => {
      ...
      const payload = await agrid.getRemoteConfigPayload('test-flag')
    ...
    it('should work without local evaluation enabled', async () => {
      ...
      const agridWithoutLocalEval = new Agrid('TEST_API_KEY', {
        host: 'http://example.com',
        fetchRetryCount: 0,
        disableCompression: true,
        personalApiKey: 'TEST_PERSONAL_API_KEY',
      ...
      const spy = jest.spyOn(agridWithoutLocalEval as any, '_requestRemoteConfigPayload')
      spy.mockResolvedValue({
        json: () => Promise.resolve({ test: 'payload' }),
      })

      const payload = await agridWithoutLocalEval.getRemoteConfigPayload('test-flag')
      expect(payload).toEqual({ test: 'payload' })
      expect(spy).toHaveBeenCalledWith('test-flag')

      // Verify that no poller was created
      expect(agridWithoutLocalEval['featureFlagsPoller']).toBeUndefined()
    })
    ...
    it('should include project API key in the remote config URL', async () => {
      ...
      await agrid.getRemoteConfigPayload('test-flag')
