...
import { __AGRID_ERROR_MESSAGES, AgridErrorBoundary } from '../AgridErrorBoundary'
import posthog from 'agrid-js'

describe('AgridErrorBoundary component', () => {
    ...
    it('should warn user when fallback is null', () => {
        ...
        expect(console.warn).toHaveBeenCalledWith(__AGRID_ERROR_MESSAGES.INVALID_FALLBACK)
    })
...
function RenderWithError({ message = 'Error', fallback, additionalProperties }: any) {
    return (
        <AgridErrorBoundary fallback={fallback} additionalProperties={additionalProperties}>
...
function RenderWithoutError({ additionalProperties }: any) {
    return (
        <AgridErrorBoundary fallback={<div></div>} additionalProperties={additionalProperties}>
