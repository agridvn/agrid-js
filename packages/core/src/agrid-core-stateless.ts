...
class AgridFetchHttpError extends Error {
  name = 'AgridFetchHttpError'

  constructor(
    public response: AgridFetchResponse,
    public reqByteLength: number
  ) {
    super('HTTP error while fetching Agrid: status=' + response.status + ', reqByteLength=' + reqByteLength)
  }
...
class AgridFetchNetworkError extends Error {
  name = 'AgridFetchNetworkError'

  constructor(public error: unknown) {
    // TRICKY: "cause" is a newer property but is just ignored otherwise. Cast to any to ignore the type issue.
    // eslint-disable-next-line @typescript-eslint/prefer-ts-expect-error
    // @ts-ignore
    super('Network error while fetching Agrid', error instanceof Error ? { cause: error } : {})
  }
}
...
export async function logFlushError(err: any): Promise<void> {
  if (err instanceof AgridFetchHttpError) {
    let text = ''
    try {
      text = await err.text
    } catch {}

    console.error(`Error while flushing Agrid: message=${err.message}, response body=${text}`, err)
  } else {
    console.error('Error while flushing Agrid', err)
  }
  return Promise.resolve()
}

function isAgridFetchError(err: unknown): err is AgridFetchHttpError | AgridFetchNetworkError {
  return typeof err === 'object' && (err instanceof AgridFetchHttpError || err instanceof AgridFetchNetworkError)
}

function isAgridFetchContentTooLargeError(err: unknown): err is AgridFetchHttpError & { status: 413 } {
  return typeof err === 'object' && err instanceof AgridFetchHttpError && err.status === 413
}
...
export abstract class AgridCoreStateless {
  // options
  ...
  protected _remoteConfigResponsePromise?: Promise<AgridRemoteConfig | undefined>
  protected _logger: Logger

  // Abstract methods to be overridden by implementations
  abstract fetch(url: string, options: AgridFetchOptions): Promise<AgridFetchResponse>
  abstract getLibraryId(): string
  abstract getLibraryVersion(): string
  abstract getCustomUserAgent(): string | void

  // This is our abstracted storage. Each implementation should handle its own
  abstract getPersistedProperty<T>(key: AgridPersistedProperty): T | undefined
  abstract setPersistedProperty<T>(key: AgridPersistedProperty, value: T | null): void

  constructor(apiKey: string, options: AgridCoreOptions = {}) {
    assert(apiKey, "You must pass your Agrid project's api key.")

    this.apiKey = apiKey
    this.host = removeTrailingSlash(options.host || 'https://app.agrid.com')
    ...
    this._logger = createLogger('[Agrid]', this.logMsgIfDebug.bind(this))
  ...
  public get optedOut(): boolean {
    return this.getPersistedProperty(AgridPersistedProperty.OptedOut) ?? !this.defaultOptIn
  }

  async optIn(): Promise<void> {
    this.wrap(() => {
      this.setPersistedProperty(AgridPersistedProperty.OptedOut, false)
    })
  }

  async optOut(): Promise<void> {
    this.wrap(() => {
      this.setPersistedProperty(AgridPersistedProperty.OptedOut, true)
    })
  }
   ...
   *
   * @remarks
   * Debug mode logs all Agrid calls to the console for troubleshooting.
  ...
  protected groupIdentifyStateless(
    ...
    options?: AgridCaptureOptions,
    distinctId?: string,
    eventProperties?: AgridEventProperties
  ): void {
    this.wrap(() => {
      const payload = this.buildPayload({
  ...
  protected async getRemoteConfig(): Promise<AgridRemoteConfig | undefined> {
    ...
    const fetchOptions: AgridFetchOptions = {
      method: 'GET',
    ...
    return this.fetchWithRetry(url, fetchOptions, { retryCount: 0 }, this.remoteConfigRequestTimeoutMs)
      .then((response) => response.json() as Promise<AgridRemoteConfig>)
      .catch((error) => {
  ...
  protected async getFlags(
    ...
    return this.fetchWithRetry(url, fetchOptions, { retryCount: 0 }, this.featureFlagsRequestTimeoutMs)
      .then((response) => response.json() as Promise<AgridV1FlagsResponse | AgridV2FlagsResponse>)
      .then((response) => normalizeFlagsResponse(response))
      .catch((error) => {
        this._events.emit('error', error)
        return undefined
      }) as Promise<AgridFlagsResponse | undefined>
  }
  ...
  ): Promise<AgridFlagsResponse['featureFlagPayloads'] | undefined> {
    await this._initPromise
  ...
  protected async getFeatureFlagsStateless(
    ...
    requestId: AgridFlagsResponse['requestId'] | undefined
  }> {
    await this._initPromise
  ...
  protected async getFeatureFlagDetailsStateless(
    ...
    disableGeoip?: boolean,
    flagKeysToEvaluate?: string[]
  ): Promise<AgridFeatureFlagDetails | undefined> {
    await this._initPromise
    ...
    if (flagsResponse.errorsWhileComputingFlags) {
      console.error(
        '[FEATURE FLAGS] Error while computing feature flags, some flags may be missing or incorrect. Learn more at https://agrid.com/docs/feature-flags/best-practices'
      )
    }
    ...
    if (flagsResponse.quotaLimited?.includes(QuotaLimitedFeature.FeatureFlags)) {
      console.warn(
        '[FEATURE FLAGS] Feature flags quota limit exceeded - feature flags unavailable. Learn more about billing limits at https://agrid.com/docs/billing/limits-alerts'
      )
      return {
        flags: {},
        featureFlags: {},
   ...
   *** SUPER PROPERTIES
   ***/
  private _props: AgridEventProperties | undefined

  protected get props(): AgridEventProperties {
    if (!this._props) {
      this._props = this.getPersistedProperty<AgridEventProperties>(AgridPersistedProperty.Props)
    }
    return this._props || {}
  }

  protected set props(val: AgridEventProperties | undefined) {
    this._props = val
  }

  async register(properties: AgridEventProperties): Promise<void> {
    this.wrap(() => {
      this.props = {
        ...this.props,
        ...properties,
      }
      this.setPersistedProperty<AgridEventProperties>(AgridPersistedProperty.Props, this.props)
    })
  }
   ...
   ***/
  protected enqueue(type: string, _message: any, options?: AgridCaptureOptions): void {
    this.wrap(() => {
      if (this.optedOut) {
        this._events.emit(type, `Library is disabled. Not sending event. To re-enable, call agrid.optIn()`)
        return
      }

      const message = this.prepareMessage(type, _message, options)

      const queue = this.getPersistedProperty<AgridQueueItem[]>(AgridPersistedProperty.Queue) || []
      ...
      queue.push({ message })
      this.setPersistedProperty<AgridQueueItem[]>(AgridPersistedProperty.Queue, queue)
  ...
  protected async sendImmediate(type: string, _message: any, options?: AgridCaptureOptions): Promise<void> {
    if (this.disabled) {
      this._logger.warn('The client is disabled')
      return
    }
  ...
  private prepareMessage(type: string, _message: any, options?: AgridCaptureOptions): AgridEventProperties {
    const message = {
      ..._message,
      type: type,
   ...
   * @public
   *
   * @throws AgridFetchHttpError
   * @throws AgridFetchNetworkError
   * @throws Error
   */
  ...
  private async _flush(): Promise<void> {
    ...
    let queue = this.getPersistedProperty<AgridQueueItem[]>(AgridPersistedProperty.Queue) || []
    ...
    while (queue.length > 0 && sentMessages.length < originalQueueLength) {
      ...
      const persistQueueChange = (): void => {
        const refreshedQueue = this.getPersistedProperty<AgridQueueItem[]>(AgridPersistedProperty.Queue) || []
        const newQueue = refreshedQueue.slice(batchItems.length)
        this.setPersistedProperty<AgridQueueItem[]>(AgridPersistedProperty.Queue, newQueue)
        queue = newQueue
      }
      ...
      const retryOptions: Partial<RetriableOptions> = {
        retryCheck: (err) => {
          ...
          if (isAgridFetchContentTooLargeError(err)) {
            return false
          }
          // otherwise, retry on network errors
          return isAgridFetchError(err)
        },
      }
      ...
      try {
        ...
        if (isAgridFetchContentTooLargeError(err) && batchMessages.length > 1) {
        ...
        if (!(err instanceof AgridFetchNetworkError)) {
          persistQueueChange()
        }
        this._events.emit('error', err)
  ...
  private async fetchWithRetry(
    url: string,
    options: AgridFetchOptions,
    retryOptions?: Partial<RetriableOptions>,
    requestTimeout?: number
  ): Promise<AgridFetchResponse> {
    ...
    return await retriable(
      async () => {
        let res: AgridFetchResponse | null = null
        try {
          ...
          throw new AgridFetchNetworkError(e)
        }
        ...
        if (!isNoCors && (res.status < 200 || res.status >= 400)) {
          throw new AgridFetchHttpError(res, reqByteLength)
        }
        return res
      },
  ...
  async _shutdown(shutdownTimeoutMs: number = 30000): Promise<void> {
    ...
    return Promise.race([
      new Promise<void>((_, reject) => {
        safeSetTimeout(() => {
          this._logger.error('Timed out while shutting down Agrid')
          hasTimedOut = true
          reject('Timeout while shutting down Agrid. Some events may not have been sent.')
        }, shutdownTimeoutMs)
      }),
      doShutdown(),
    ])
