...
import { AgridCustomAppProperties, AgridCustomStorage } from './types'
...
export const getAppProperties = (): AgridCustomAppProperties => {
  const properties: AgridCustomAppProperties = {
...
const buildLegacyStorage = (filesystem: any): AgridCustomStorage => {
  return {
...
export const buildOptimisiticAsyncStorage = (): AgridCustomStorage => {
  ...
  throw new Error(
    'Agrid: No storage available. Please install expo-file-system or react-native-async-storage OR implement a custom storage provider.'
  )
}
