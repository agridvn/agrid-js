...
const REACT_NATIVE_TYPE_EXAMPLES = {
    AgridEventProperties: `// Properties for React Native events
{
    ...
    AgridEventProperty: `// Can be a string or an object
"user@example.com" | { name: "John", age: 25 }`
};

const config = {
    packageDir: path.resolve(__dirname, '..'),  // packages/agrid-react-native
    apiJsonPath: path.resolve(__dirname, '../docs/agrid-react-native.api.json'),
    outputPath: path.resolve(__dirname, `../references/agrid-react-native-references-${version}.json`),
    version: version,
...
const versionedPath = path.resolve(__dirname, `../references/agrid-react-native-references-${version}.json`);
fs.writeFileSync(versionedPath, JSON.stringify(output, null, 2));

// Copy to latest file
const latestPath = path.resolve(__dirname, '../references/agrid-react-native-references-latest.json');
