...
import { AgridExceptions } from './agrid-exceptions'
...
/* agrid.init is called with `Partial<AgridConfig>`
 ...
 * The OnlyValidKeys type ensures that only keys that are valid in the AgridConfig type are allowed.
 */
type OnlyValidKeys<T, Shape> = T extends Shape ? (Exclude<keyof T, keyof Shape> extends never ? T : never) : never

const instances: Record<string, Agrid> = {}

// some globals for comparisons
const __NOOP = () => {}

const PRIMARY_INSTANCE_NAME = 'agrid'
...
export const defaultConfig = (defaults?: ConfigDefaults): AgridConfig => ({
    ...
    debug: (location && isString(location?.search) && location.search.indexOf('__agrid_debug=true') !== -1) || false,
...
export const configRenames = (origConfig: Partial<AgridConfig>): Partial<AgridConfig> => {
    const renames: Partial<AgridConfig> = {}
...
class DeprecatedWebPerformanceObserver {
    ...
    set _forceAllowLocalhost(value: boolean) {
        logger.error(
            'WebPerformanceObserver is deprecated and has no impact on network capture. Use `_forceAllowLocalhostNetworkCapture` on `agrid.sessionRecording`'
        )
...
/**
 *
 * This is the SDK reference for the Agrid JavaScript Web SDK.
 ...
 * to integrate Agrid into your project.
 *
 * This SDK is designed for browser environments.
 * Use the Agrid [Node.js SDK](/docs/libraries/node) for server-side usage.
 *
 * @constructor
 */
export class Agrid {
    __loaded: boolean
    config: AgridConfig
    _originalUserConfig?: Partial<AgridConfig>

    rateLimiter: RateLimiter
    scrollManager: ScrollManager
    pageViewManager: PageViewManager
    featureFlags: AgridFeatureFlags
    surveys: AgridSurveys
    experiments: WebExperiments
    toolbar: Toolbar
    exceptions: AgridExceptions
    consent: ConsentManager

    // These are instance-specific state created after initialisation
    persistence?: AgridPersistence
    sessionPersistence?: AgridPersistence
    ...
    constructor() {
        ...
        this.featureFlags = new AgridFeatureFlags(this)
        this.toolbar = new Toolbar(this)
        this.scrollManager = new ScrollManager(this)
        this.pageViewManager = new PageViewManager(this)
        this.surveys = new AgridSurveys(this)
        this.experiments = new WebExperiments(this)
        this.exceptions = new AgridExceptions(this)
    ...
    /**
     * Initializes a new instance of the Agrid capturing object.
     *
     * @remarks
     ...
     * ```js
     * // multiple instances
     * agrid.init('<ph_project_api_key>', {}, 'project1')
     * agrid.init('<ph_project_api_key>', {}, 'project2')
     * ```
     *
     * @public
     *
     * @param token - Your Agrid API token
     * @param config - A dictionary of config options to override
     * @param name - The name for the new agrid instance that you want created
     *
    ...
    init(token: string, config?: OnlyValidKeys<Partial<AgridConfig>, Partial<AgridConfig>>, name?: string): Agrid {
        if (!name || name === PRIMARY_INSTANCE_NAME) {
            ...
            const namedAgrid = instances[name] ?? new Agrid()
            namedAgrid._init(token, config, name)
            instances[name] = namedAgrid

            // Add as a property to the primary instance (this isn't type-safe but it is how it was always done)
            ;(instances[PRIMARY_INSTANCE_NAME] as any)[name] = namedAgrid

            return namedAgrid
        }
    }

    // agrid._init(token:string, config:object, name:string)
    //
    // This function sets up the current instance of the agrid
    ...
    //
    _init(token: string, config: Partial<AgridConfig> = {}, name?: string): Agrid {
        ...
        if (this.__loaded) {
            ...
            console.warn('[Agrid.js]', 'You have already initialized Agrid! Re-initializing is a no-op')
            return this
        }

        this.__loaded = true
        this.config = {} as AgridConfig // will be set right below
        ...
        this.persistence = new AgridPersistence(this.config, persistenceDisabled)
        this.sessionPersistence =
            this.config.persistence === 'sessionStorage' || this.config.persistence === 'memory'
                ? this.persistence
                : new AgridPersistence({ ...this.config, persistence: 'sessionStorage' }, persistenceDisabled)
    ...
    private _processInitTaskQueue(queue: Array<() => void>, initStartTime: number): void {
        ...
        if (this.config.__preview_deferred_init_extensions) {
            logger.info(`Agrid extensions initialized (${taskInitTiming}ms)`)
        }
    }
    ...
    /**
     * _execute_array() deals with processing any agrid function
     * calls that were called before the Agrid library were loaded
     * (and are thus stored in an array so they can be called later)
     *
     * Note: we fire off all the agrid function calls && user defined
     * functions BEFORE we fire off agrid capturing calls. This is so
    ...
    _execute_array(array: SnippetArrayItem[]): void {
        ...
        eachArray(array, (item) => {
            if (item) {
                fn_name = item[0]
                if (isArray(fn_name)) {
                    capturing_calls.push(item) // chained call e.g. agrid.get_group().set()
     ...
     * ```js
     * // basic event capture
     * agrid.capture('cta-button-clicked', {
    ...
    capture(
        ...
        if (!this.__loaded || !this.persistence || !this.sessionPersistence || !this._requestQueue) {
            logger.uninitializedWarning('agrid.capture')
            return
        }
        ...
        if (isUndefined(event_name) || !isString(event_name)) {
            logger.error('No event name provided to agrid.capture')
            return
        }
        ...
        if (properties?.$current_url && !isString(properties?.$current_url)) {
            logger.error(
                'Invalid `$current_url` property provided to `agrid.capture`. Input must be a string. Ignoring provided value.'
            )
        ...
        // This doesn't handle $set_once, because agrid-people doesn't either
    ...
    /**
     * This method is used internally to calculate the event properties before sending it to Agrid. It can also be
     * used by integrations (e.g. Segment) to enrich events with Agrid properties before sending them to Segment,
     * which is required for some Agrid products to work correctly. (e.g. to have a correct $session_id property).
     *
     ...
     * agrid.register({ campaign: 'summer_sale' }, 7) // expires in 7 days
     * ```
     *
     * @public
     *
     ...
     * agrid.unregister_for_session('current_flow')
     * ```
     *
     * @public
     *
     ...
     * ```js
     * // check boolean flag
     * if (agrid.getFeatureFlag('new-feature')) {
     *     // show new feature
     * }
     * ```
     *
     ...
     * @param {Object|String} options (optional) If {send_event: false}, we won't send an $feature_flag_call event to Agrid.
     */
     ...
     * @example
     * ```js
     * if(agrid.getFeatureFlag('beta-feature') === 'some-value') {
     *      const someValue = agrid.getFeatureFlagPayload('beta-feature')
     *      // do something
     * }
     * ```
     *
     ...
     * if (agrid.isFeatureEnabled('new-checkout')) {
     *     showNewCheckout()
     * }
     * ```
     *
     ...
     * if (agrid.isFeatureEnabled('feature', { send_event: false })) {
     ...
     *   )) {
     *     agrid.updateEarlyAccessFeatureEnrollment(
     *       betaKey,
     *       false
     *     )
     *     setActiveBetas(
     ...
     *
     * useEffect(() => {
     *   agrid.getEarlyAccessFeatures((features) => {
    ...
    /**
     * Exposes a set of events that Agrid will emit.
     ...
     * @example
     * ```js
     * agrid.onFeatureFlags(function(featureFlags, featureFlagsVariants, { errorsLoading }) {
     *     // do something
     * })
     * ```
     *
     ...
     *
     * Callback parameters:
     * - surveys: Survey[]: An array containing all survey objects fetched from Agrid using the getSurveys method
     ...
     * @example
     * ```js
     * agrid.onSurveysLoaded((surveys, context) => { // do something })
     * ```
     *
     *
     ...
     *
     * Can be used, for example, to sync the Agrid session id with a backend session.
     *
     * {@label Identification}
     *
     * @example
     * ```js
     * agrid.onSessionId(function(sessionId, windowId) { // do something })
     * ```
     *
     ...
     * }
     *
     * agrid.getSurveys(callback, false)
     * ```
     *
     * @public
     *
     ...
     * @example
     * ```js
     * agrid.getActiveMatchingSurveys((surveys) => {
     *      // do something
     * })
     * ```
     *
     ...
     * @example
     * ```js
     * agrid.renderSurvey(coolSurveyID, '#survey-container')
     * ```
     *
     ...
     * agrid.displaySurvey('survey-id-123')
     * ```
     *
     * @example
     * ```js
     ...
     * @example
     * ```js
     * agrid.canRenderSurveyAsync(surveyId).then((result) => {
     *     if (result.visible) {
     ...
     *
     * @remarks
     * By default, Agrid assigns each user a randomly generated `distinct_id`. Use this method to
     * replace that ID with your own unique identifier (like a user ID from your database).
     *
     * @example
     * ```js
     * // basic identification
     * agrid.identify('user_12345')
     * ```
     *
     * @example
     * ```js
    ...
    identify(new_distinct_id?: string, userPropertiesToSet?: Properties, userPropertiesToSetOnce?: Properties): void {
        if (!this.__loaded || !this.persistence) {
            return logger.uninitializedWarning('agrid.identify')
        }
        if (isNumber(new_distinct_id)) {
            new_distinct_id = (new_distinct_id as number).toString()
            logger.warn(
                'The first argument to agrid.identify was a number, but it should be a string. It has been converted to a string.'
            )
        }

        //if the new_distinct_id has not been set ignore the identify event
        if (!new_distinct_id) {
            logger.error('Unique user id has not been set in agrid.identify')
            return
        }

        if (isDistinctIdStringLike(new_distinct_id)) {
            logger.critical(
                `The string "${new_distinct_id}" was set in agrid.identify which indicates an error. This ID should be unique to the user and not a hardcoded string.`
            )
            return
        }
        if (new_distinct_id === COOKIELESS_SENTINEL_VALUE) {
            logger.critical(
                `The string "${COOKIELESS_SENTINEL_VALUE}" was set in agrid.identify which indicates an error. This ID is only used as a sentinel value.`
            )
            return
        }

        if (!this._requirePersonProcessing('agrid.identify')) {
            return
        }
     ...
     *
     * @remarks
     * Updates user properties that are stored with the person profile in Agrid.
    ...
    setPersonProperties(userPropertiesToSet?: Properties, userPropertiesToSetOnce?: Properties): void {
        ...
        if (!this._requirePersonProcessing('agrid.setPersonProperties')) {
            return
        }
     ...
     * agrid.group('organization', 'org_12345', {
     *     name: 'Acme Corp',
     *     plan: 'enterprise'
     * })
     * ```
    ...
    group(groupType: string, groupKey: string, groupPropertiesToSet?: Properties): void {
        if (!groupType || !groupKey) {
            logger.error('agrid.group requires a group type and group key')
            return
        }

        if (!this._requirePersonProcessing('agrid.group')) {
            return
        }
     ...
     * agrid.setPersonPropertiesForFlags({'property1': 'value', property2: 'value2'}, false)
     * ```
     *
     * @public
     *
     ...
     * agrid.setGroupPropertiesForFlags({'organization': { name: 'CYZ', employees: '11' } })
     * ```
     *
     * @example
     * ```js
     * // Set properties without reload
     * agrid.setGroupPropertiesForFlags({'organization': { name: 'CYZ', employees: '11' } }, false)
     * ```
     *
    ...
    setGroupPropertiesForFlags(properties: { [type: string]: Properties }, reloadFeatureFlags = true): void {
        if (!this._requirePersonProcessing('agrid.setGroupPropertiesForFlags')) {
            return
        }
     ...
     * agrid.reset(true)  // also resets device_id
     * ```
     *
     * @public
     */
    reset(reset_device_id?: boolean): void {
        logger.info('reset')
        if (!this.__loaded) {
            return logger.uninitializedWarning('agrid.reset')
        }
        ...
        this.register(
            {
                $last_agrid_reset: new Date().toISOString(),
            },
            1
        )
    }
     ...
     * The distinct ID is used to associate events with users in Agrid.
     *
     * {@label Identification}
     *
     * @example
     ...
     * const userId = agrid.get_distinct_id()
     ...
     * agrid.init('token', {
     *     loaded: (agrid) => {
     *         const id = agrid.get_distinct_id()
     *         // use the ID
     *     }
     * })
     * ```
     ...
     * ```js
     * // basic usage
     * agrid.get_session_replay_url()
     *
     * @example
     * ```js
     * // timestamp
     * agrid.get_session_replay_url({ withTimestamp: true })
     * ```
     *
     * @example
     * ```js
     ...
     *
     * @remarks
     * Agrid will use this to link two distinct_ids going forward (not retroactively).
     ...
     * agrid.alias('user_12345')
     * ```
     *
     * @example
     * ```js
     * // explicit alias with original ID
     * agrid.alias('user_12345', 'anonymous_abc123')
     * ```
     *
     * @public
     *
    ...
    alias(alias: string, original?: string): CaptureResult | void | number {
        ...
        // agrid.people.identify() call made for this user. It is VERY BAD to make an alias with
        // this ID, as it will duplicate users.
        if (alias === this.get_property(PEOPLE_DISTINCT_ID_KEY)) {
            logger.critical('Attempting to create alias for existing People user - aborting.')
            return -2
        }
        if (!this._requirePersonProcessing('agrid.alias')) {
            return
        }
    ...
    /**
     * Updates the configuration of the Agrid instance.
     *
     * {@label Initialization}
     *
     * @public
     *
     * @param {Partial<AgridConfig>} config A dictionary of new configuration values to update
     */
    set_config(config: Partial<AgridConfig>): void {
        ...
        if (isObject(config)) {
            ...
            this.sessionPersistence =
                this.config.persistence === 'sessionStorage' || this.config.persistence === 'memory'
                    ? this.persistence
                    : new AgridPersistence({ ...this.config, persistence: 'sessionStorage' }, isPersistenceDisabled)
     ...
     * agrid.startSessionRecording(true)
     * ```
     *
     * @example
     * ```js
     * // Start and override controls
     * agrid.startSessionRecording({
     ...
     * if (agrid.sessionRecordingStarted()) {
     *   agrid.stopSessionRecording()
     * }
     * ```
     */
     ...
     *
     * @remarks
     * get_property() can only be called after the Agrid library has finished loading.
     * init() has a loaded function available to handle this automatically.
     *
     * @example
     * ```js
     * // grab value for '$user_id' after the agrid library has loaded
     * agrid.init('<YOUR PROJECT TOKEN>', {
     *     loaded: function(agrid) {
     *         user_id = agrid.get_property('$user_id');
     *     }
     * });
     * ```
     * @public
     ...
     *
     * @remarks
     * This is based on browser-level `sessionStorage`, NOT the Agrid session.
     * getSessionProperty() can only be called after the Agrid library has finished loading.
     * init() has a loaded function available to handle this automatically.
     *
     * @example
     * ```js
     ...
     * agrid.init('YOUR PROJECT TOKEN', {
     *     loaded: function(agrid) {
     *         user_id = agrid.getSessionProperty('user_id');
     *     }
     * });
     * ```
     *
    ...
    /**
     * Returns a string representation of the Agrid instance.
     *
     * {@label Initialization}
     *
     * @internal
    ...
    createPersonProfile(): void {
        ...
        if (!this._requirePersonProcessing('agrid.createPersonProfile')) {
            return
        }
     ...
     *
     * @remarks
     * Enables event tracking and data persistence (cookies/localStorage) for this Agrid instance.
     ...
     *
     * @remarks
     * Disables event tracking and data persistence (cookies/localStorage) for this Agrid instance.
     ...
     * @example
     * ```js
     * if (agrid.has_opted_in_capturing()) {
     ...
     * @example
     * ```js
     * if (agrid.has_opted_out_capturing()) {
     ...
     * @example
     * ```js
     * const consentStatus = agrid.get_explicit_consent_status()
    ...
    /**
     * Checks whether the Agrid library is currently capturing events.
     *
     ...
     * {@label Privacy}
     *
     * @see {AgridConfig.cookieless_mode}
     * @see {AgridConfig.opt_out_persistence_by_default}
     * @see {AgridConfig.respect_dnt}
     *
     * @returns {boolean} whether the agrid library is capturing events
     */
    ...
    /**
     * Clear the user's opt in/out status of data capturing and cookies/localstorage for this Agrid instance
     *
     * {@label Privacy}
     *
     * @public
     ...
     *
     * @remarks
     * Debug mode logs all Agrid calls to the browser console for troubleshooting.
     * Can also be enabled by adding `?__agrid_debug=true` to the URL.
     *
     * {@label Initialization}
     *
     * @example
...
const add_dom_loaded_handler = function () {
    ...
    function dom_loaded_handler() {
        ...
        each(instances, function (inst: Agrid) {
            inst._dom_loaded()
        })
    }
    ...
    // so let's simply log an error stating Agrid couldn't be initialized
    // We're checking for `window` to avoid erroring out on a SSR context
    if (window) {
        logger.error("Browser doesn't support `document.addEventListener` so Agrid couldn't be initialized")
    }
}

export function init_from_snippet(): void {
    const agridMain = (instances[PRIMARY_INSTANCE_NAME] = new Agrid())

    const snippetAgrid = assignableWindow['agrid']

    if (snippetAgrid) {
        /**
         * The snippet uses some clever tricks to allow deferred loading of array.js (this code)
         *
         * window.agrid is an array which the queue of calls made before the lib is loaded
         * It has methods attached to it to simulate the agrid object so for instance
         *
         * window.agrid.init("TOKEN", {api_host: "foo" })
         * window.agrid.capture("my-event", {foo: "bar" })
         *
         * ... will mean that window.agrid will look like this:
         * window.agrid == [
         *  ["my-event", {foo: "bar"}]
         * ]
         *
         * window.agrid[_i] == [
         *   ["TOKEN", {api_host: "foo" }, "agrid"]
         * ]
         *
         * If a name is given to the init function then the same as above is true but as a sub-property on the object:
         *
         * window.agrid.init("TOKEN", {}, "ph2")
         * window.agrid.ph2.people.set({foo: "bar"})
         *
         * window.agrid.ph2 == []
         * window.agrid.people == [
         *  ["set", {foo: "bar"}]
        ...
        each(snippetAgrid['_i'], function (item: [token: string, config: Partial<AgridConfig>, name: string]) {
            if (item && isArray(item)) {
                const instance = agridMain.init(item[0], item[1], item[2])

                const instanceSnippet = snippetAgrid[item[2]] || snippetAgrid
    ...
    assignableWindow['agrid'] = agridMain

    add_dom_loaded_handler()
}

export function init_as_module(): Agrid {
    const agridMain = (instances[PRIMARY_INSTANCE_NAME] = new Agrid())
