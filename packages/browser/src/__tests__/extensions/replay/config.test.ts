import { defaultConfig } from '../../../agrid-core'
...
describe('config', () => {
    describe('network request options', () => {
        describe('maskRequestFn', () => {
            ...
            it('client can force disable recording', () => {
                const agridConfig = defaultConfig()
                agridConfig.session_recording.recordHeaders = false
                agridConfig.session_recording.recordBody = false
                const networkOptions = buildNetworkRequestOptions(agridConfig, {
                    recordHeaders: true,
                    recordBody: true,
                })
            ...
            it('uses the deprecated mask fn when set', () => {
                const agridConfig = defaultConfig()
                agridConfig.session_recording.maskNetworkRequestFn = (data) => {
                    return {
                        ...data,
            ...
            it.each([
                ...
                const networkOptions = buildNetworkRequestOptions(
                    { ...defaultConfig(), api_host: apiHost || 'https://us.agrid.com' },
                    {}
                )
    ...
    describe('masking/privacy', () => {
        ...
        it('can amend the provided object', () => {
            const agridConfig = defaultConfig()
            agridConfig.session_recording.maskCapturedNetworkRequestFn = (data) => {
                data.name = 'changed'
                return data
            }
            const networkOptions = buildNetworkRequestOptions(agridConfig, {})
