...
export class AgridFeatureFlags {
     ...
     * ### Usage:
     *
     *     if(agrid.getFeatureFlag('my-flag') === 'some-variant') { // do something }
     *
     * @param {Object|String} key Key of the feature flag.
     * @param {Object|String} options (optional) If {send_event: false}, we won't send an $feature_flag_called event to Agrid.
     */
     ...
     * value from the Agrid API.
     *
     * Note: Because the agrid-js SDK is primarily used with public project API keys, encrypted remote config payloads will
     ...
     * ### Usage:
     *
     *     if(agrid.isFeatureEnabled('beta-feature')) { // do something }
     *
     * @param key Key of the feature flag.
     * @param [options] If {send_event: false}, we won't send an $feature_flag_call event to Agrid.
     ...
     * or for testing/debugging feature flags in the Agrid app.
     *
     * ### Usage:
     *
     *     - agrid.featureFlags.overrideFeatureFlags(false) // clear all overrides
     *     - agrid.featureFlags.overrideFeatureFlags(['beta-feature']) // enable flags
     *     - agrid.featureFlags.overrideFeatureFlags({'beta-feature': 'variant'}) // set variants
     *     - agrid.featureFlags.overrideFeatureFlags({ // set both flags and payloads
     *         flags: {'beta-feature': 'variant'},
     *         payloads: { 'beta-feature': { someData: true } }
     *       })
     *     - agrid.featureFlags.overrideFeatureFlags({ // only override payloads
     *         payloads: { 'beta-feature': { someData: true } }
     *       })
     */
    overrideFeatureFlags(overrideOptions: OverrideFeatureFlagsOptions): void {
        if (!this._instance.__loaded || !this._instance.persistence) {
            return logger.uninitializedWarning('agrid.featureFlags.overrideFeatureFlags')
        }

        // Clear all overrides if false, lets you do something like agrid.featureFlags.overrideFeatureFlags(false)
        ...
        if (
            ...
            // Handle flags if provided, lets you do something like agrid.featureFlags.overrideFeatureFlags({flags: ['beta-feature']})
            if ('flags' in options) {
            ...
            // Handle payloads independently, lets you do something like agrid.featureFlags.overrideFeatureFlags({payloads: { 'beta-feature': { someData: true } }})
     ...
     * ### Usage:
     *
     *     agrid.onFeatureFlags(function(featureFlags, featureFlagsVariants, { errorsLoading }) { // do something })
     *
