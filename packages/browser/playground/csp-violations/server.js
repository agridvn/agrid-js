...
if (!process.env.AGRID_TOKEN || !process.env.AGRID_API_HOST) {
    throw new Error('AGRID_TOKEN and AGRID_API_HOST must be set')
}

// UPDATE YOUR TOKEN!!!
const AGRID_TOKEN = process.env.AGRID_TOKEN
const AGRID_API_HOST = removeTrailingSlash(process.env.AGRID_API_HOST)
const AGRID_UI_HOST = removeTrailingSlash(process.env.AGRID_UI_HOST || AGRID_API_HOST)
const AGRID_USE_SNIPPET = process.env.AGRID_USE_SNIPPET === 'true' || process.env.AGRID_USE_SNIPPET === '1'

const AGRID_SCRIPT = AGRID_USE_SNIPPET
    ? `<script>
    !function(t,e){var o,n,p,r;e.__SV||(window.agrid=e,e._i=[],e.init=function(i,s,a){function g(t,e){var o=e.split(".");2==o.length&&(t=t[o[0]],e=o[1]),t[e]=function(){t.push([e].concat(Array.prototype.slice.call(arguments,0)))}}(p=t.createElement("script")).type="text/javascript",p.crossOrigin="anonymous",p.async=!0,p.src=s.api_host.replace(".i.agrid.com","-assets.i.agrid.com")+"/static/array.js",(r=t.getElementsByTagName("script")[0]).parentNode.insertBefore(p,r);var u=e;for(void 0!==a?u=e[a]=[]:a="agrid",u.people=u.people||[],u.toString=function(t){var e="agrid";return"agrid"!==a&&(e+="."+a),t||(e+=" (stub)"),e},u.people.toString=function(){return u.toString(1)+".people (stub)"},o="init bs ws ge fs capture De calculateEventProperties $s register register_once register_for_session unregister unregister_for_session Is getFeatureFlag getFeatureFlagPayload isFeatureEnabled reloadFeatureFlags updateEarlyAccessFeatureEnrollment getEarlyAccessFeatures on onFeatureFlags onSurveysLoaded onSessionId getSurveys getActiveMatchingSurveys renderSurvey canRenderSurvey canRenderSurveyAsync identify setPersonProperties group resetGroups setPersonPropertiesForFlags resetPersonPropertiesForFlags setGroupPropertiesForFlags resetGroupPropertiesForFlags reset get_distinct_id getGroups get_session_id get_session_replay_url alias set_config startSessionRecording stopSessionRecording sessionRecordingStarted captureException loadToolbar get_property getSessionProperty xs Ss createPersonProfile Es gs opt_in_capturing opt_out_capturing has_opted_in_capturing has_opted_out_capturing clear_opt_in_out_capturing ys debug ks getPageViewId captureTraceFeedback captureTraceMetric".split(" "),n=0;n<o.length;n++)g(u,o[n]);e._i.push([i,s,a])},e.__SV=1)}(document,window.agrid||[]);
    agrid.init("${AGRID_TOKEN}", {
        api_host: "${AGRID_API_HOST}",
        ui_host: "${AGRID_UI_HOST}",
    })
</script>`
    : `<script src="/dist/agrid.js"></script>`

const CSP_REPORT_URI = `${AGRID_API_HOST}/report/?token=${AGRID_TOKEN}`
const USE_REPORT_TO = AGRID_API_HOST.startsWith('https://')

const CSP_RULES = {
    'default-src': "'self'",
    'script-src': `'self' ${AGRID_API_HOST} ${AGRID_USE_SNIPPET ? "'unsafe-inline'" : "'nonce-123'"}`,
    'connect-src': `'self' ${AGRID_API_HOST} ${AGRID_UI_HOST} https://*.agrid.com`,
    'img-src': "'self' data:",
    'style-src': `'self' ${AGRID_UI_HOST}`,
    'report-uri': CSP_REPORT_URI + '&type=report-uri',
    ...(USE_REPORT_TO
        ? {
              'report-to': 'agrid', // easier to debug with report-uri
          }
        : {}),
}
...
const CSP_HEADER = Object.entries(CSP_RULES)
    ...
    .join('; ')
const REPORTING_HEADER = `agrid="${CSP_REPORT_URI}&type=report-to"`
...
app.get('/debug-enabled', (req, res) => {
    ...
    if (USE_REPORT_TO) {
        res.setHeader('Reporting-Endpoints', `agrid="${CSP_REPORT_URI}&type=report-to&debug=true"`)
    }
...
app.get('/report-uri-only', (req, res) => {
    const reportUriOnlyRules = {
        'default-src': "'self'",
        'script-src': `'self' ${AGRID_API_HOST} 'nonce-123'`,
        'connect-src': `'self' ${AGRID_API_HOST} ${AGRID_UI_HOST} https://*.agrid.com`,
        'img-src': "'self' data:",
        'style-src': `'self' ${AGRID_UI_HOST}`,
...
app.get('/report-to-only', (req, res) => {
    ...
    res.setHeader('Reporting-Endpoints', `agrid-debug="${CSP_REPORT_URI}&type=report-to-only&debug=true"`)
    ...
    <body>
      ...
      <div class="card">
        ...
        <p>Reporting Endpoints: <code>agrid-debug="${CSP_REPORT_URI}&type=report-to-only&debug=true"</code></p>
        <p><a href="/">Back to Home</a></p>
      </div>
      <!-- This inline script violates CSP -->
      <script>
...
app.get('/both-report-directives', (req, res) => {
    ...
    res.setHeader('Reporting-Endpoints', `agrid-both="${CSP_REPORT_URI}&type=both-report-to&debug=true"`)
    ...
    <body>
      ...
      <div class="card">
        ...
        <p>Reporting Endpoints: <code>agrid-both="${CSP_REPORT_URI}&type=both-report-to&debug=true"</code></p>
        <p><a href="/">Back to Home</a></p>
      </div>
      <!-- This inline script violates CSP -->
      <script>
