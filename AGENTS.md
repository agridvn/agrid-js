# Agrid JS SDK Development Guide

## Overview

This is a pnpm monorepo containing multiple Agrid JavaScript SDKs and development tooling. The repository uses Turbo for build orchestration and supports local development through tarball-based testing.
...
| ---------------- | ------------------------ | ----------------------------------------------- |
| `browser/`       | `agrid-js`             | Main browser SDK for capturing events and usage |
| `web/`           | `agrid-js-lite`        | Lightweight browser SDK                         |
...
- Example and playground projects are independent pnpm workspaces. You can install their dependencies by running `pnpm install` inside the specific project folder. All dependencies and sub-dependencies to Agrid SDKs will be overwritten using a pnpmfile.
...
4. you might need to point package.json at the tarball like `"agrid-js": "file:../../target/agrid-js.tgz",`
...
- Custom `eslint-plugin-agrid-js` for repository-specific rules
- TypeScript support
- React support
